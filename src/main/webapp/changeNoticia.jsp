	...
	<form action="${pageContext.request.contextPath}/editar-noticia"
		...
		<div class="mb-3">
			<label for="titulo" class="form-label">Título</label> <input
				...
				value="${noticiaSelecionada.titulo}" required>
		</div>
		<div class="mb-3">
			<label for="nomeAutor" class="form-label">Autor</label> <input
				type="text" class="form-control" id="nomeAutor" name="nomeAutor"
				value="${noticiaSelecionada.nomeAutor}" required>
		</div>
		<div class="mb-3">
		...
		<div class="mb-3">
			<label for="dataPublicacao" class="form-label">Data de
				Publicação</label> <input type="date" class="form-control"
				id="dataPublicacao" name="dataPublicacao"
				value="${noticiaSelecionada.dataPublicacao}" required>
		</div>
		<div class="mb-3">
		...
		<c:if test="${noticiaSelecionada.imagem ne null}">
			<%
			byte[] img = (byte[]) request.getAttribute("noticiaSelecionada").getClass().getMethod("getImagem")
					.invoke(request.getAttribute("noticiaSelecionada"));
			String base64 = java.util.Base64.getEncoder().encodeToString(img);
			%>
			<div class="mb-3">
				<label class="form-label">Imagem Atual</label><br> <img
					src="data:image/png;base64,<%=base64%>" alt="Imagem da notícia"
					class="img-thumbnail mb-3" style="max-width: 200px;" />
			</div>
		</c:if>

		<div class="mb-3">
			<label for="imagem" class="form-label">Alterar Imagem de
				Destaque</label> <input type="file" class="form-control" id="imagem"
